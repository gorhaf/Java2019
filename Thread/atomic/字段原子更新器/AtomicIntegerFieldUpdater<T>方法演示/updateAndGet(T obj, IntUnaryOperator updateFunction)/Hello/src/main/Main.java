...
public class Main {
    ...
    public static void main(String[] args) {
        ...
        int result = fieldUpdater.getAndUpdate(person, new IntUnaryOperator() {
            @Override
        ...
        // 输出结果
        System.out.println("getAndUpdate(T obj, IntUnaryOperator updateFunction)方法执行结果：" + result);
        // updateAndGet(T obj, IntUnaryOperator updateFunction)方法的作用是更新指定对象的字段，更新方式由参数updateFunction决定，方法返回更新后的值。
        result = fieldUpdater.updateAndGet(person, new IntUnaryOperator() {
            @Override
            public int applyAsInt(int operand) {
                return operand * 2;
            }
        ...
        // 输出结果
        System.out.println("updateAndGet(T obj, IntUnaryOperator updateFunction)方法执行结果：" + result);
    }
}
