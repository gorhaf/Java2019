...
public class Main {
    ...
    public static void main(String[] args) {
        ...
        if (rwl.hasQueuedThreads()) {
            // 输出语句
            System.out.println("有" + rwl.getQueueLength() + "个线程正在获取读写锁。");

            // 分别判断我们创建的3个线程是否正在等待获取读写锁
            System.out.println("thread1是否正在等待获取读写锁: " + rwl.hasQueuedThread(thread1));
            System.out.println("thread2是否正在等待获取读写锁: " + rwl.hasQueuedThread(thread2));
            System.out.println("thread3是否正在等待获取读写锁: " + rwl.hasQueuedThread(thread3));
        }
    }
}
/*
